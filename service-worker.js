...
self.addEventListener("fetch", (event) => {
  ...
  if (url.includes('/api/') || url.includes('json')) {
    ...
    // In dev, prefer the network for the app shell so code/style changes show up immediately,
    // but keep a cache fallback so a refresh does not re-download everything if the network is slow.
    debugLog('Handling dev app-shell request with network-first:', url);
...
async function networkFirst(request) {
  try {
    ...
    if (cacheCopy.ok && cacheCopy.type === 'basic') {
      caches.open(CACHE_NAME).then(cache => {
    ...
    debugLog('Network-first request failed, falling back to cache:', request.url, error);
